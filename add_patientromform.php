...
$sqlfreerooms = "SELECT Room_id FROM rooms ";
$sqlfreeroomresult = mysqli_query($db, $sqlfreerooms);

$sqlfreepatient = "SELECT Patient_id,Patient_Name FROM patients p";
$sqlfreepatientresult = mysqli_query($db, $sqlfreepatient);
?>
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="table-responsive">
                       <form action="add_patientrom.php" method="post" class="form-inline">

                       <label class="col-lg-6">Patient Name</label>
                       <select name="prid" class="form-control">
                       <?php
                        while($row = mysqli_fetch_assoc($sqlfreepatientresult)) 
                        {
                            $pid = $row['Patient_id'];
                            $pname = $row['Patient_Name'];

                            echo "<option value = $pid>$pname\n";
                            }
                            ?>
                            </select><br>

                       <label class="col-lg-6">Room ID</label>
                       <select name="freeroom" class="form-control">
                       <?php
                       while($row = mysqli_fetch_assoc($sqlfreeroomresult)) 
                        {
                            $rid = $row['Room_id'];
            
                            echo "<option value = $rid>$rid\n";
                            }
